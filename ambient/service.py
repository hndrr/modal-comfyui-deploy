...
class JobService:
    ...
    def submit(self, raw):
        ...
        job = {'id': job_id, 'status': 'queued', 'stage': 'Queued', 'request': request,
               'fingerprint': fingerprint(request), 'createdAt': self.now()}
        ...
        try:
            ...
            job.update(status='failed', stage='Dispatch failed', error='Job dispatch failed; create a new request to retry.')
    ...
    def get(self, job_id):
        ...
        if job['status'] == 'queued' and self.now()-job['createdAt'] > 300 and not self.store.get('call:'+job_id):
            job = {**job, 'status': 'failed', 'error': 'Dispatch acknowledgement missing. Inspect Modal before retrying.'}
        if job['status'] in ('queued', 'running') and self.reconcile and self.now()-job['createdAt'] > 10:
            ...
            if call_id:
                ...
                if reason:
                    ...
                    if fresh['status'] in ('queued', 'running'):
                        job = {**fresh, 'status': 'failed', 'stage': 'Worker stopped', 'error': reason}
                    else:
                        job = fresh
