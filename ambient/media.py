...
def finalize(source: Path, destination: Path, final_frame: Path, clip_id: str) -> dict:
    ...
    info = json.loads(subprocess.check_output([
        ...
        '-of', 'json', str(temporary)], timeout=60))
    ...
    run(['ffmpeg', '-nostdin', '-v', 'error', '-y', '-i', str(temporary),
         '-vf', f'select=eq(n\\,{frames-1})', '-frames:v', '1', str(final_frame)])
    ...
    return {'id': clip_id, 'duration': duration, 'fps': float(numerator)/float(denominator),
            'width': video['width'], 'height': video['height'], 'frames': frames,
            'hasAudio': True, 'bytes': destination_tmp_size}
